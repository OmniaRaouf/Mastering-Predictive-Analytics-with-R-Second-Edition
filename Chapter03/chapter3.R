...
machine <- read.csv(paste0(here(),"/data/machine.data"), header = F)
names(machine) <- c("VENDOR", "MODEL", "MYCT", "MMIN", 
  "MMAX", "CACH", "CHMIN", "CHMAX", "PRP", "ERP")
machine <- machine[, 3:9]
head(machine, n = 3)
# checking the missing values on the data frame
library(purrr)
map_df(machine, function(x) sum(is.na(x)))
...
machine_train_features <- machine[machine_sampling_vector, 1:6]
...
par(mar=c(1,1,1,1)) # to avoide figure margins too large
par(mfrow = c(2, 1))
...
#using built in functions to diagnose the results
plot(machine_model1)
plot(cars_model2)
