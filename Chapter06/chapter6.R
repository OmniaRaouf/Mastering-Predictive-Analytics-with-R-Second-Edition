...
bdf<- read.table(paste0(here(),"/data/biodeg.csv"), sep = ";", quote = "\"")
head(bdf, n = 3)
...
model_radial_improved<- svm(V42 ~ ., data = bdf_train, kernel = "radial", 
                   cost = 10, gamma = 0.01)
#0.9183431953
# 0.8904761905
mean(bdf_train[,42] == model_radial_improved$fitted)

test_improved_predictions<- predict(model_radial_improved, bdf_test[,1:41])
mean(bdf_test[,42] == test_improved_predictions)
...
german_raw<- read.table(paste0(here(),"/data/german.data"), quote = "\"")
